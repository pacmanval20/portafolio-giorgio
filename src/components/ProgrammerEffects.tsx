...
export default function ProgrammerEffects() {
  ...
  useEffect(() => {
    ...
    const measure = () => { words.forEach(item => wordTops.set(item, layoutTop(item))); };

    // ── Entrance animations — one-shot per element, distinct per type ──
    const entranceItems = Array.from(document.querySelectorAll<HTMLElement>(
      '.section-heading, .live-terminal, .chapter-word, .tech-chapter > p, .tech-chapter .tags, ' +
      '.nubo-scene, .project-body, .about > div, .journey li, ' +
      '.contact h2, .contact > p, .contact .actions, .faq details'
    ));
    entranceItems.forEach(item => item.classList.add('entrance-item'));
    // Stagger delays for grouped list elements
    document.querySelectorAll<HTMLElement>('.journey li').forEach((el, i) => el.style.setProperty('--entrance-delay', String(i * 0.1)));
    document.querySelectorAll<HTMLElement>('.faq details').forEach((el, i) => el.style.setProperty('--entrance-delay', String(i * 0.07)));
    ...
    const draw = (now: number) => {
      ...
      // ── Cursor position (diamond only, no halo canvas) ──
      if (showPointer && fine.matches && !mobile.matches && pointer.current) {
    ...
    // ── Click: color flash en el cursor ──
    const clickDown = (e: PointerEvent) => { if (fine.matches && !mobile.matches && e.pointerType === 'mouse') pointer.current?.classList.add('is-clicking'); };
    const clickUp = () => pointer.current?.classList.remove('is-clicking');
    ...
    resize();
    for (let i = 0; i < population(); i++) { spawn(); meteors[i].age = Math.random() * meteors[i].life * .8; }
    updateScenes();
    raf = requestAnimationFrame(draw);
    ...
    return () => {
      ...
      observeCode.disconnect();
      layoutObserver.disconnect();
      entranceObserver.disconnect();
      entranceItems.forEach(item => { item.classList.remove('entrance-item', 'in-view'); item.style.removeProperty('--entrance-delay'); });
      document.querySelectorAll<HTMLElement>('.journey li, .faq details').forEach(el => el.style.removeProperty('--entrance-delay'));
      wordGroups.forEach(({items}) => items.forEach(word => word.style.removeProperty('--word-progress')));
      codePanel?.classList.remove('is-typing');
      hide();
      ...
      ctx?.clearRect(0, 0, width, height);
      root.classList.remove('fx-on', 'fx-sleep', 'hero-in-view');
  ...
  return <>
    ...
    <div className="binary-cursor" ref={pointer} aria-hidden="true"><span /></div>
